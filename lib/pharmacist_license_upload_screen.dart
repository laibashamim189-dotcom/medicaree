...
class _PharmacistLicenseUploadScreenState extends State<PharmacistLicenseUploadScreen> {
  ...
  Future<void> _submitLicense() async {
    ...
    if (_certificateFile == null && (existingUrl == null || existingUrl.toString().isEmpty)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Please upload your Drug License Certificate image")),
      );
      return;
    }
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('users').doc(user.uid).update({
        'drugLicenseNumber': _drugLicenseController.text.trim(),
        'pharmacistRegNumber': _pharmacistRegController.text.trim(),
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (_drugLicenseController.text.isEmpty && userData['drugLicenseNumber'] != null) {
          _drugLicenseController.text = userData['drugLicenseNumber'];
        }
        if (_pharmacistRegController.text.isEmpty && userData['pharmacistRegNumber'] != null) {
          _pharmacistRegController.text = userData['pharmacistRegNumber'];
        }
        if (_pharmacyNameController.text.isEmpty && userData['pharmacyName'] != null) {
          _pharmacyNameController.text = userData['pharmacyName'];
        }
        if (_pharmacyAddressController.text.isEmpty && userData['pharmacyAddress'] != null) {
          _pharmacyAddressController.text = userData['pharmacyAddress'];
        }
              ...
              child: Column(
                children: [
                  ...
                  TextFormField(
                    ...
                    enabled: isEditable,
                    decoration: const InputDecoration(labelText: "Pharmacy Name", border: OutlineInputBorder(), prefixIcon: Icon(Icons.local_pharmacy)),
                  ),
                  const SizedBox(height: 15),
                  TextFormField(
                    controller: _pharmacyAddressController,
                    enabled: isEditable,
                    decoration: const InputDecoration(labelText: "Pharmacy Address", border: OutlineInputBorder(), prefixIcon: Icon(Icons.map)),
                  ),
                  const SizedBox(height: 15),
                  TextFormField(
                    controller: _drugLicenseController,
                    enabled: isEditable,
                    decoration: const InputDecoration(labelText: "Drug License Number", border: OutlineInputBorder(), prefixIcon: Icon(Icons.description)),
                  ),
                  const SizedBox(height: 15),
                  TextFormField(
                    controller: _pharmacistRegController,
                    enabled: isEditable,
                    decoration: const InputDecoration(labelText: "Pharmacist Registration Number", border: OutlineInputBorder(), prefixIcon: Icon(Icons.how_to_reg)),
                  ),
                  const SizedBox(height: 20),
                  
                  const Align(alignment: Alignment.centerLeft, child: Text("Drug License Certificate Copy", style: TextStyle(fontWeight: FontWeight.bold))),
                  const SizedBox(height: 10),
